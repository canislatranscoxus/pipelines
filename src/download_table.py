...
usage
python -m download_table --input stg_mty_metro_area  --output ~/data_lake/out/stg_mty_metro_area 
...
def run(argv=None, save_main_session=True):
  ...
  with beam.Pipeline(options=pipeline_options) as p:
    ...
    (
        ...
        #| "dump json" >> beam.Map( lambda j: json.dumps( j ) )
        | "WriteToText" >> beam.io.WriteToText(known_args.output, file_name_suffix=".txt", shard_name_template="")
    )
